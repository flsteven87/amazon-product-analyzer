...
class LangGraphAgent:
        ...
        if self._connection_pool is None:
            try:
                ...
                self._connection_pool = AsyncConnectionPool(
                    settings.POSTGRES_URL_LANGGRAPH,
                    open=False,
                    max_size=max_size,
                    kwargs={
                        "autocommit": True,
    ...
    async def get_response(
        ...
        try:
            # Only pass the latest user message to avoid duplication with checkpointer
            # LangGraph's add_messages will handle combining with existing state
            latest_message = messages[-1] if messages else None
            if not latest_message or latest_message.role != "user":
                raise ValueError("Last message must be from user")
            
            response = await self._graph.ainvoke(
                {"messages": [latest_message.model_dump()], "session_id": session_id}, config
            )
